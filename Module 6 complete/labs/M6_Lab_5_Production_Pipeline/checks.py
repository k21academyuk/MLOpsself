...
def run_drift_detection(reference: pd.DataFrame, current: pd.DataFrame) -> tuple:
    ...
    # Lower the dataset-drift threshold to 10% so a realistic ~20%-of-columns shift
    # (the monsoon batch) trips a dataset-level alert. Evidently's default is 50%.
    drift_share_threshold = float(os.environ.get(
        "DRIFT_SHARE_THRESHOLD", str(getattr(config, "DRIFT_SHARE_THRESHOLD", 0.10))))
    ...
    md = report.as_dict()["metrics"]

    # metrics[0] = DatasetDriftMetric summary. NOTE its "drift_share" field is the THRESHOLD,
    # not the fraction drifted — use "share_of_drifted_columns" for the actual fraction.
    # Per-column detail lives on the DataDriftTable metric, NOT DatasetDriftMetric, so we grab
    # whichever metric in the report actually carries "drift_by_columns" (fixes the KeyError).
    summary = md[0]["result"]
